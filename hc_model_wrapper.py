...
def run(retsim_dir=None,vid_dir=None,expt='hc_bulb',filename=None,errfilename=None,d=None,R=False,channel_default=True,vid_args=None,pov_args=None,pov_fn='temp',pov_in_fn=None,pov_image='temp.png',povres=3000,recolor=0,print_err=0,**kwargs):
    ...
    hb_file    = kwargs.get('hb_file',None)
    hb_densfile = kwargs.get('hb_densfile',None)
    ...
    checker_inten = kwargs.get('checker_inten',None)
    checker_contrast = kwargs.get('checker_contrast',None)
    pulsedur   = kwargs.get('pulsedur',None)
    pulsegap   = kwargs.get('pulsegap',None)
    cone_timec = kwargs.get('cone_timec',None)
    drm        = kwargs.get('drm',None)
    dcrm       = kwargs.get('dcrm',None)
    dri        = kwargs.get('dri',None)
    vnoise     = kwargs.get('vnoise',0)
    dnoise     = kwargs.get('dnoise',0)
    pnoise     = kwargs.get('pnoise',0)
    Chnoise     = kwargs.get('Chnoise',0)
    cnoise     = kwargs.get('cnoise',0)
    ...
    xstimsize = kwargs.get('xstimsize',None)
    ystimsize = kwargs.get('ystimsize',None)
    xpixels = kwargs.get('xpixels',None)
    ypixels = kwargs.get('ypixels',None)
    xstimoff = kwargs.get('xstimoff',None)
    ystimoff = kwargs.get('ystimoff',None)

    mxrot = kwargs.get('mxrot',None)
    myrot = kwargs.get('myrot',None)
    mzrot = kwargs.get('mzrot',None)    
    ...
    if hb_file!=None:     arguments+=['--hb_file',hb_file]
    if hb_densfile!=None: arguments+=['--hb_densfile',hb_densfile,'--hb_densfile2',hb_densfile]
    ...
    if checker_inten!=None:    arguments+=['--checker_inten',str(checker_inten)]
    if checker_contrast!=None:    arguments+=['--checker_contrast',str(checker_contrast)]
    if pulsedur!=None:    arguments+=['--pulsedur',str(pulsedur)]
    if pulsegap!=None:    arguments+=['--pulsegap',str(pulsedur)]
    if cone_timec!=None:  arguments+=['--cone_timec',str(cone_timec)]
    ...
    if dnoise!=None:      arguments+=['--dnoise',str(dnoise)]
    if pnoise!=None:      arguments+=['--pnoise',str(pnoise)]
    if Chnoise!=None:     arguments+=['--Chnoise',str(Chnoise)]
    if cnoise!=None:      arguments+=['--cnoise',str(cnoise)]
    ...
    if cone_timec!=None:  arguments+=['--cone_timec',str(cone_timec)]
    if xstimsize!=None:   arguments+=['--xstimsize',str(xstimsize)]
    if ystimsize!=None:   arguments+=['--ystimsize',str(ystimsize)]
    if xpixels!=None:     arguments+=['--xpixels',str(xpixels)]
    if ypixels!=None:     arguments+=['--ypixels',str(ypixels)]
    if xstimoff!=None:    arguments+=['--xstimoff',str(xstimoff)]
    if ystimoff!=None:    arguments+=['--ystimoff',str(ystimoff)]

    if mxrot!=None:       arguments+=['--mxrot',str(mxrot)]
    if myrot!=None:       arguments+=['--myrot',str(myrot)]
    if mzrot!=None:       arguments+=['--mzrot',str(mzrot)]
    ...
    if R: #render povray image
        ...
        if recolor==2:
            with open(retsim_dir+pov_fn+'.pov','r') as f:
                pov_file=f.read() 
            pov_file=pov_file.replace('nc.pov','nc_w.pov')
            pov_file=pov_file.replace('Magenta','Grey')
            pov_file=pov_file.replace('Red','Magenta*0.3')
            pov_file=pov_file.replace('Cyan','Magenta*0.3')
            pov_file=pov_file.replace('Blue','Magenta*0.3')    
            pov_file=pov_file.replace('Green','Cyan*0.7')
            with open(retsim_dir+pov_fn+'.pov','w') as f:
                f.write(pov_file)
        ...
        if pov_args:
            ...
            pov_arg_list+=['+h'+str(povres),'+w'+str(povres),'+i'+pov_in_fn+'.pov','+o'+pov_image]
